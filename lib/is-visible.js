define(function(require, exports, module){
    ...
    var getWindowInfo = function(){
        return {x: 0, y: $(window).scrollTop(), width:$(window).width(), height: $(window).height()};
    }
    ...
    var isVisible = function(ele, pos){
        ...
        if(elePosition == 'fixed'){
            return !!isOverlap(pos ? pos: getInfo($ele), getWindowInfo());
        ...
        if(ele.nodeName != 'BODY'){
            switch(elePosition){
                ...
                case 'relative':
                    ...
                    if('hidden|scroll|auto'.indexOf(parentOverflow) >= 0 && $parent.get(0).nodeName != 'BODY'){
                        var p = null;
                        if(p = isOverlap(pos ? pos: getInfo($ele), getInfo($parent))){
                            return isVisible($parent, p);
                        }else{
                            return false;
                        }
                    }else{
                        if($parent.get(0).nodeName == 'BODY'){
                            var p = null;
                            if(p = isOverlap(pos ? pos: getInfo($ele), getWindowInfo())){
                                return isVisible($parent, p);
                            }else{
                                return false;
                            }
                        }else{
                            return isVisible($parent, pos? pos : getInfo($ele));
                        }
                    }
                    break;
                case 'absolute':
